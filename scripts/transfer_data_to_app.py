'''
1. remote records of analyser/app/app.db's case_backup table
2. query record from ./app.db's case_info 
3. insert them to case_backup table
'''
import config
import sqlite3
...
def main():
    ...
    # delete records fo case_backup table
    web_c.execute('delete from case_backup')
    print('finish clean case_backup table')

    # insert record to case_backup table
    for sub in ret:
        sql = build_sql(sub)
...
def build_sql(single):
    ...
    return "INSERT INTO case_backup ('{}', '{}', '{}', '{}', '{}', '{}', '{}') VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}')"\
...
''' case_backup & case_info table sctructure
sqlite> .schema case_backup
